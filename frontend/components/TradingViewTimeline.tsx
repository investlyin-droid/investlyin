...
interface TradingViewTimelineProps {
  ...
  feedMode?: 'all_symbols' | 'watchlist';
...
function TradingViewTimeline({
  ...
  const widgetContainerIdRef = useRef<string>(`tradingview-timeline-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`);
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      script.onerror = () => {
        console.error('[TradingView] Failed to load timeline widget script');
        scriptLoadedRef.current = false;
      };
  ...
  }, [displayMode, feedMode, colorTheme, isTransparent, locale, width, height]);
